    ...
    public class State1Initial : State
    {
        ...
        public override Screen GetScreen(int screenNumber)
        {
            switch (screenNumber)
            {
                ...
                case (int)Screens.Details2:
                    {
                        var strLine1 = "=  Start new brew 2=";
                        var strLine2 = "Boil tm: " + BrewData.Config.BoilTime + "min";
                        var strLine3 = "";
                        var strLine4 = "";
        ...
        {
            if (GetCurrentScreenNumber == (int)Screens.Details1)
            {
                RiseStateChangedEvent(new State2Warmup(BrewData));
            }
            if (GetCurrentScreenNumber == (int)Screens.Details2)
            {
                RiseStateChangedEvent(new State2Warmup(BrewData));
            }
        ...
        protected override void StartExtra()
        {
            ...
            BrewData.BrewBoilStart = DateTime.MinValue;
            BrewData.BrewCooldownStart = DateTime.MinValue;
            BrewData.BrewComplete = DateTime.MinValue;
